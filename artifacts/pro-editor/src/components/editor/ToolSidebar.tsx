...
const TOOLS: { id: ActiveTool; icon: React.ReactNode; label: string; shortcut?: string; group: string }[] = [
  ...
  { id: "gradient", icon: <Blend size={14} />, label: "Grad", shortcut: "G", group: "shape" },
    ...
    <TooltipProvider delayDuration={300}>
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            {tool.icon}
            <span style={{ fontSize: "8px", lineHeight: 1 }}>{tool.label}</span>
          </button>
        </TooltipTrigger>
        <TooltipContent side="right" style={{ fontSize: "12px", background: "#0d0b22", border: "1px solid rgba(139,92,246,0.25)" }}>
          <div style={{ fontWeight: 700 }}>{tool.label}</div>
          {tool.shortcut && <div style={{ color: "rgba(255,255,255,0.4)", fontSize: "11px" }}>Key: {tool.shortcut}</div>}
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
...
export default function ToolSidebar() {
  ...
  return (
    <div style={{
      width: "52px",
      background: "linear-gradient(180deg, #060412 0%, #04020e 100%)",
      borderRight: "1px solid rgba(139,92,246,0.1)",
      display: "flex", flexDirection: "column", alignItems: "center",
      paddingTop: "8px", paddingBottom: "8px", gap: "2px",
      flexShrink: 0, overflowY: "auto", overflowX: "hidden",
      boxShadow: "inset -1px 0 0 rgba(139,92,246,0.05)",
    }}>
      {/* Tools by group */}
      {groups.map((group, gi) => (
        <div key={group} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "2px", width: "100%", padding: "0 5px" }}>
          {gi > 0 && (
            <div style={{ width: "28px", height: "1px", background: "rgba(139,92,246,0.12)", margin: "4px 0" }} />
          )}
          <div style={{ fontSize: "6px", color: "rgba(255,255,255,0.18)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "2px", width: "100%", textAlign: "center", marginBottom: "2px" }}>
            {GROUP_LABELS[group]}
          </div>
      ...
      {["brush", "eraser", "dodge", "burn"].includes(activeTool) && (
        <div style={{ width: "100%", padding: "0 5px" }}>
          <div style={{ width: "28px", height: "1px", background: "rgba(139,92,246,0.12)", margin: "4px auto" }} />
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "4px" }}>
            <div
              ...
              style={{
                ...
                boxShadow: `0 0 12px ${brushColor}66`,
              }}
            />
            <div style={{ fontSize: "7px", color: "rgba(255,255,255,0.3)", fontFamily: "monospace" }}>{brushSize}px</div>
          </div>
        </div>
      )}
      ...
      {/* Panels grouped */}
      <div style={{ width: "100%", padding: "0 5px", paddingBottom: "4px" }}>
        <div style={{ width: "28px", height: "1px", background: "rgba(139,92,246,0.15)", margin: "6px auto 8px" }} />

        {panelGroups.map((group, gi) => (
          <div key={group} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "2px", width: "100%", marginBottom: "4px" }}>
            {gi > 0 && <div style={{ width: "22px", height: "1px", background: "rgba(255,255,255,0.05)", margin: "2px auto" }} />}
            <div style={{
              fontSize: "5.5px", color: GROUP_COLORS[group] || "rgba(255,255,255,0.18)",
              fontWeight: 700, textTransform: "uppercase", letterSpacing: "1.5px",
              width: "100%", textAlign: "center", marginBottom: "2px", opacity: 0.6,
            }}>
              {group}
            </div>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      className={`tool-btn w-9 ${activePanel === p.id ? "active" : ""}`}
                    >
                      {p.icon}
                      <span style={{ fontSize: "7.5px" }}>{p.label}</span>
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="right" style={{ fontSize: "12px", background: "#0d0b22", border: "1px solid rgba(139,92,246,0.25)" }}>
                    {p.label} Panel
        ...
        <div style={{ width: "28px", height: "1px", background: "rgba(139,92,246,0.1)", margin: "6px auto" }} />
        <TooltipProvider delayDuration={300}>
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                onClick={toggleKeyboardShortcuts}
                className="tool-btn w-9"
                style={{ color: "rgba(255,255,255,0.2)" }}
                onMouseOver={e => (e.currentTarget.style.color = "#a78bfa")}
                onMouseOut={e => (e.currentTarget.style.color = "rgba(255,255,255,0.2)")}
              >
                <Keyboard size={13} />
                <span style={{ fontSize: "7.5px" }}>Keys</span>
              </button>
            </TooltipTrigger>
            <TooltipContent side="right" style={{ fontSize: "12px", background: "#0d0b22", border: "1px solid rgba(139,92,246,0.25)" }}>
              Keyboard Shortcuts (?)
